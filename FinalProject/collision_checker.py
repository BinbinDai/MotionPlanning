    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # circle_locations[:, 1] = ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = path[0][j] + np.multiply(self._circle_offsets, cos(path[2][j]))
                circle_locations[:, 1] = path[1][j] + np.multiply(self._circle_offsets, sin(path[2][j]))
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # --------------------------------------------------------------
                path = paths[i]
                score = np.sqrt((path[0][-1] - goal_state[0])**2 + (path[1][-1] - goal_state[1])**2)
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # score += self._weight * ...
                            # --------------------------------------------------
                            collide_path = paths[j] 
                            min_dist = min([np.sqrt((path[0][k] - collide_path[0][k])**2 + (path[1][k] - collide_path[1][k])**2) for k in range(len(path))])
