...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
      #  pass
        ...
        # return thetas
        # ------------------------------------------------------------------
        thetas = np.multiply(a, s) + np.divide(b, 2) * np.power(s, 2) + np.divide(c, 3)*np.power(s, 3) + np.divide(d, 4)*np.power(s, 4) 
        return thetas 
    ...
    def sample_spiral(self, p):
        ...
        a_points = [a]*len(s_points)
        b_points = [b]*len(s_points)
        c_points = [c]*len(s_points)
        d_points = [d]*len(s_points)
